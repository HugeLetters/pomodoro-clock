...
export const pomodoroSchema = z.object({
  name: z.coerce.string({ invalid_type_error: "Value must be a string" }).min(1),
  session: z.coerce.number({ invalid_type_error: "Value must be an integer" }).min(1).int(),
  pause: z.coerce.number({ invalid_type_error: "Value must be an integer" }).min(1).int(),
  selected: z.boolean(),
  id: z.string().min(1),
});
